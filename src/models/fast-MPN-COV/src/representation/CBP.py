...
class CBP(nn.Module):
     ...
     def forward(self, x):
         ...
         for img in range(batchSize // bsn):
             ...
             x_padded = torch.cat((sketch1, torch.zeros(sketch1.size(), device=sketch1.device)), dim=2)
             sketch1 = torch.fft.rfft(x_padded, dim=2)  # Use fft.rfft in 1.8+, dim=2 for channels
             
             sketch2 = batch_x.mm(self.sparseM[1].to(x.device)).unsqueeze(2)
             x_padded = torch.cat((sketch2, torch.zeros(sketch1.size(), device=sketch2.device)), dim=2)
             sketch2 = torch.fft.rfft(x_padded, dim=2)  # Use fft.rfft in 1.8+, dim=2 for channels

             Re = sketch1[:, :, 0].mul(sketch2[:, :, 0]) - sketch1[:, :, 1].mul(sketch2[:, :, 1])
             Im = sketch1[:, :, 0].mul(sketch2[:, :, 1]) + sketch1[:, :, 1].mul(sketch2[:, :, 0])

             tmp_y = torch.fft.ifft(torch.cat((Re.unsqueeze(2), Im.unsqueeze(2)), dim=2), 1)[:, :, 0]

             y[interSmall, :] = tmp_y.view(torch.numel(interSmall), h, w, self.output_dim).sum(dim=1).sum(dim=1).real
