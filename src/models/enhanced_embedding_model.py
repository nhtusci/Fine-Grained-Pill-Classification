...
class EnhancedEmbeddingModel(nn.Module):
    ...
    def forward(self, x, return_domain_logits=False):
        ...
        if hasattr(self.base_model, 'features'):
            ...
            # Flatten
            x = x.view(x.size(0), -1)
            # Classifier (trong base_model nếu có) - sử dụng classifier
            # để giảm chiều của representation (vd. BCNN: dr^2 -> cont_dims)
        ...
        # Domain classification (only during training when return_domain_logits=True)
