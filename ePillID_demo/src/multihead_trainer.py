...
def custom_collate_fn(batch):
    """Custom collate function để xử lý tensor integers như is_ref, is_front"""
    images = torch.stack([item['image'] for item in batch])
    labels = torch.tensor([item['label'] for item in batch], dtype=torch.long)
    image_names = [item['image_name'] for item in batch]
    is_ref = torch.tensor([item['is_ref'] for item in batch], dtype=torch.bool)
    ...
    # Add is_front if present
    if 'is_front' in batch[0]:
        is_front = torch.tensor([item['is_front'] for item in batch], dtype=torch.bool)
        data_dict['is_front'] = is_front
    
    return data_dict


def create_dataloaders(args, ref_only_df, cons_train_df, cons_val_df,
    ...
    dataloaders = {x: torch.utils.data.DataLoader(
        ...
        pin_memory=True,
        collate_fn=custom_collate_fn) for x in ['train', 'val']}
...
def hneg_train_model(model, optimizer, scheduler,
                ...
                grl_scheduler=None,  # Thêm cái này nếu bạn dùng Domain Adaptation
                args=None
                ):
    ...
    for epoch in range(num_epochs):
        ...
        # Update GRL lambda nếu model hỗ trợ domain adaptation
        if args and getattr(args, 'use_domain_adaptation', False):
            from models.grl_domain_classifier import compute_lambda
            lambda_ = compute_lambda(epoch, num_epochs)
            # Cập nhật lambda cho tất cả domain classifiers
            if hasattr(model, 'embedding_model') and hasattr(model.embedding_model, 'domain_classifier'):
                model.embedding_model.domain_classifier.set_lambda(lambda_)
            print(f"Domain GRL lambda: {lambda_:.4f}")
        ...
        for phase in ['train', 'val']:
            ...
            for batch_index, batch_data in enumerate(pbar):
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    # Trích domain logits nếu có (chỉ dùng domain logits trong training)
                    domain_logits = all_outputs.get('domain_logits', None)

                    # Chuẩn bị is_ref (đảm bảo nó lên GPU nếu cần) - only relevant for training
                    is_ref = batch_data.get('is_ref', None)
                    if phase != 'train':
                        # Do not pass domain info during validation/eval to avoid spurious warnings
                        domain_logits = None
                        is_ref = None
                    else:
                        if is_ref is not None and not is_ref.is_cuda:
                            is_ref = is_ref.to(device)

                    # Tính Loss (bao gồm domain loss nếu domain_logits có)
                    loss_outputs = criterion(all_outputs, labels, 
                                            is_front=batch_data.get('is_front', None), 
                                            is_ref=is_ref,
                                            domain_logits=domain_logits)

                    if loss_outputs is None:
                        warnings.warn(f"loss_outputs is None, skip this minibatch. labels: {labels}, is_front: {batch_data.get('is_front', None)}, is_ref: {batch_data.get('is_ref', None)}")
                        continue
                ...
                for k in ['loss', 'metric_loss', 'ce', 'arcface', 'contrastive', 'triplet', 'focal', 'domain']:
                    if k in loss_outputs:
...
def train(ref_only_df,
    ...
    loss_weights = {'ce': args.ce_w, 'arcface': args.arcface_w, 'contrastive': args.contrastive_w, 'triplet': args.triplet_w, 'focal': args.focal_w, 'domain': args.domain_w if args.use_domain_adaptation else 0.0 }# THÊM DÒNG NÀY}
