...
def common_parser():
    ...
    parser.add_argument('--data_root_dir', default="ePillID_data")
    ...
    # ==================== THÊM VÀO ĐÂY - DÒNG 43 ====================
    # Coordinate Attention Arguments
    parser.add_argument('--use_coord_attention', default='1', type=strtobool,
                       help='Enable Coordinate Attention after backbone for better spatial feature focus')
    parser.add_argument('--ca_reduction', type=int, default=32,
                       help='Reduction ratio for Coordinate Attention (higher = less parameters)')
    
    # Domain Adaptation Arguments  
    parser.add_argument('--use_domain_adaptation', default='1', type=strtobool,
                       help='Enable Domain Adaptation with GRL to reduce domain gap between ref/consumer images')
    parser.add_argument('--domain_w', default=0.5, type=float,
                       help='Weight for domain adaptation loss (0.3-0.7 recommended)')
    parser.add_argument('--domain_hidden_dim', type=int, default=256,
                       help='Hidden dimension for domain classifier')
    parser.add_argument('--domain_dropout', type=float, default=0.5,
                       help='Dropout rate for domain classifier')
    # ================================================================
...
def cv_parser():
    ...
    parser.add_argument('--folds_csv_dir', default="ePillID_data/folds/pilltypeid_nih_sidelbls0.01_metric_5folds/base/", help='folder that contains data-splits csv files and encoder')
